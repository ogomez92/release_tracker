<script lang="ts">
	...
	import type { GitHubUserRepo } from '$lib/types';

	let username = $state('');
	let loading = $state(false);
	let addingRepos = $state(false);
	let fetchedRepos = $state<GitHubUserRepo[]>([]);
	...
	async function handleSearch() {
		...
		try {
			fetchedRepos = await window.electronAPI.fetchUserRepos(username.trim());
...
<section aria-labelledby="username-heading">
	<h2 id="username-heading">Add by Username</h2>
	<p>Enter a GitHub username to list their repositories.</p>

	<div class="search-controls">
		<div class="input-group">
			<label for="username-input" class="visually-hidden">GitHub username</label>
			<input
				id="username-input"
				type="text"
				bind:value={username}
			...
			/>
			<button type="button" onclick={handleSearch} disabled={loading || !username.trim()}>
	...
	{#if error}
		<p id="username-error" class="error-message" role="alert">{error}</p>
	{/if}

	{#if loading}
		<div role="status" aria-live="polite" class="loading-container">
			<span class="loading"></span>
			<span>Fetching repositories for {username}...</span>
		</div>
	{/if}

	{#if repos().length > 0}
		<div role="region" aria-labelledby="username-results-heading" aria-live="polite" class="scan-results">
			<div class="results-header">
				<h3 id="username-results-heading">
	...
	{:else if fetchedRepos.length > 0 && repos().length === 0}
		<p class="empty-state">All {fetchedRepos.length} repositories from "{username}" are already tracked.</p>
	{:else if username && !loading && !error}
		<p class="empty-state">No repositories found for user "{username}".</p>
	{/if}
</section>
